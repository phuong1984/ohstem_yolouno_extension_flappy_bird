...
BIRD_SIZE = 4
PIPE_WIDTH = 10
PIPE_GAP = 20  # Khoảng cách giữa hai phần ống
MIN_PIPE_HEIGHT = 10
GRAVITY = 0.2
JUMP_VELOCITY = -2
PIPE_SPEED = 2
...
class FlappyBird():
    ...
    def draw(self):
        ...
        for x, gap_y in self.pipes:
            ...
            # Ống dưới
            self.oled.fill_rect(x, gap_y + PIPE_GAP, PIPE_WIDTH, SCREEN_HEIGHT - (gap_y + PIPE_GAP), 1)
        # Vẽ điểm số
    ...
    def add_pipe(self):
        """Thêm cột ống mới"""
        gap_y = random.randint(MIN_PIPE_HEIGHT, SCREEN_HEIGHT - PIPE_GAP - MIN_PIPE_HEIGHT)
    ...
    def play(self):        
        if(self.game_state == 0):
            ...
            self.oled.text(f"Score: {self.score}", 30, 40, 1)
            self.oled.show()            
